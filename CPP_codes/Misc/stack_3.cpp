...
void towerOfHanoi(int n, MinStack<Height> &From, MinStack<Height> &To, MinStack<Height> &Aux, char from, char to, char aux){
    if (n == 0) {
        return;
    }
...
int main(){
    int size, ch = 1, depth, feet, inch;
    cout << "Enter the size of the stacks" <<endl;
    cin >> size;
    
    Height h, ele;

    MinStack <Height> A(size), B(size), C(size);
    while (1){
